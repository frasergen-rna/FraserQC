...
p1 <- p0 + geom_bar(aes(x=posi, y=content, fill=base), stat="identity", width=0.8)

p2 <- p1 + scale_fill_brewer(palette="Set1") + scale_x_discrete() + theme_bw() + xlab("position in read(bp)") + ylab("base content (%)") + theme(axis.title.x = element_text(face="bold", size=15), axis.title.y = element_text(face="bold", size=15), axis.text.x = element_text(angle = 45, hjust = 1,size=11), axis.text.y = element_text(size=15)) + ggtitle(paste("base content across all bases for ", strsplit(contentFile, "/")[[1]][length(strsplit(contentFile, "/")[[1]])-1])) + theme(plot.title = element_text(hjust = 0.5))
